...
class Pct(nn.Module):
    def __init__(self, args, output_channels=40):
        ...
        self.linear1 = nn.Linear(960, 512, bias=False)
    ...
    def forward(self, x):
        ...
        cls_tokens1 = self.cls_token.expand(batch_size, -1, -1)   #[[b, 64, 1]
        x = torch.cat((cls_tokens1, feature_0), dim=2)    #[[b, 64, 512]
        ...
        cls_tokens2 = self.mlp01(feature1.permute(0, 2, 1)[:,:,0]).view(batch_size,128,1)
        x1 = self.sa1(torch.cat((cls_tokens2, feature_1), dim=2))  #[b, 128, 257]
        ...
        cls_tokens3 = self.mlp12(feature2.permute(0, 2, 1)[:, :, 0]).view(batch_size, 256, 1)
        ...
        x2 = self.sa2(torch.cat((cls_tokens3, feature_2), dim=2))
        ...
        cls_tokens4 = self.mlp23(feature3.permute(0, 2, 1)[:, :, 0]).view(batch_size, 512, 1)
        ...
        x3 = self.sa3(torch.cat((cls_tokens4, feature_3), dim=2))


        #cls_logit3 = x3[:, :, 0]     #[b, 512]    直接用2维进行mlp网络
        cls_logit = torch.cat((cls_tokens1,cls_tokens2,cls_tokens3,cls_tokens4), dim=1).view(batch_size,960)

        cls_logit = F.leaky_relu(self.bn6(self.linear1(cls_logit)), negative_slope=0.2)   #[b, 256]
