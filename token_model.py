...
class Pct(nn.Module):
    ...
    def forward(self, x):
        ...
        cls_tokens = torch.cat((feature1.permute(0, 2, 1)[:,:,0].view(batch_size,64,1), F.adaptive_max_pool1d(feature1.permute(0, 2, 1)[:, :, 1:], 1)), dim=1)
        #cat((b, 64, 513)[:,:,0], MAP(b, 64, 513)[:, :, 1:])  →  cat([b, 64, 1] , [b, 64, 1]) →  [b, 128, 1]
